...
class AlarmReceiver : BroadcastReceiver() {
    ...
    private val GROUP_MESSAGE: String = "TASKLIST"
    ...
    override fun onReceive(context: Context?, intent: Intent?) {
        ...
        taskListDatabase?.toDoListDao()?.isShownUpdate(id = dbId, isShow = 1)

        val list = taskListDatabase?.toDoListDao()?.get(dbId)
    ...
    private fun initiateDatabase(context: Context) {
        if (taskListDatabase == null)
            taskListDatabase = TaskListDatabase.getInstance(context)
    }
